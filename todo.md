...
- [X] Fixed DuplicateOutputError on startup: Resolved conflict where multiple callbacks targeted `analysis-loading-indicator` with the same input. Replaced the manual loading indicator with `dcc.Loading` wrapping the results container, which is cleaner and avoids callback conflicts.
